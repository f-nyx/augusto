...
class IndexManager(indexPath: String) {
    ...
    fun index(document: Document) {
        ...
        indexWriter.addDocument(LuceneDocument().apply {
            add(LuceneField(ID_FIELD, document.id, STRING_FIELD))
            add(LuceneField(NAMESPACE_FIELD, document.namespace, STRING_FIELD))
            ...
            document.fields.forEach { field ->
                ...
                add(LuceneField("${field.name}!!$FIELD_TYPE", field.type.name, STRING_FIELD))
            }
        })
    }
    ...
    private fun transform(
        ...
        return PaginatedResult.new(
            ...
            next = if (resolvedDocs.isNotEmpty()) {
                ...
                Cursor(nextDoc.doc, nextDoc.score, nextDoc.shardIndex)
            } else {
                null
            }
        )
